 ...
 *
 * <p>
 * Supports both v1 (flat) and v2 (nested) event formats with seamless backward
 * compatibility.
 * Uses FraudEventDeserializer for version-aware deserialization and
 * TransactionEventMapper
 * for transformation to CanonicalFraudEvent.
 * </p>
 *
 ...
 * <p>
 * The FraudEventDeserializer automatically detects the schema version from the
 * "schemaVersion" field in the JSON payload and deserializes to the appropriate
 * format.
 * </p>
 *
...
public class KafkaConsumerService {
    ...
    /**
     * Consume transaction events from fraud-transactions topic (v1 - Legacy
     * Format).
     *
     * <p>
     * This listener handles legacy flat TransactionEvent format for backward
     * compatibility.
     * Events are transformed to CanonicalFraudEvent before processing.
     * </p>
     *
     * <p>
    ...
    // log.info("Received v1 transaction event: txnId={}, customerId={}, amount={},
    // partition={}, offset={}",
    // event.getTxnId(), event.getCustomerId(), event.getAmount(), partition,
    // offset);
    ...
    // transactionEventMapper.fromLegacy(event);
    // log.debug("Transformed v1 event to canonical format: txnId={},
    // schemaVersion={}",
    // canonicalEvent.getTxnId(), canonicalEvent.getSchemaVersion());

    // // Process using canonical format (preparing for Phase 5)
    // FraudDecision decision =
    // fraudOrchestratorService.analyzeTransaction(canonicalEvent);
    ...
    // } catch (Exception e) {
    // log.error("Error processing v1 transaction event: txnId={}",
    // event.getTxnId(), e);
    // throw new RuntimeException("Failed to process v1 transaction event for
    // txnId=" + event.getTxnId(), e);
    // }
    // }

    /**
     * Consume transaction events from fraud-transactions topic (v2 - Nested
     * Format).
     *
     * <p>
     * This listener handles the new nested CanonicalFraudEvent format with rich
     * contextual data.
     * This is the preferred format for new integrations.
     * </p>
     *
     * <p>
     * Version detection is handled automatically by FraudEventDeserializer based on
     * the
     * "schemaVersion" field in the JSON payload.
     * </p>
     *
     * <p>
     * Valid events are retried by the listener error handler on processing failures
     ...
     * @param acknowledgment Manual acknowledgment
     */
    @KafkaListener(topics = "${kafka.topics.fraud-transactions:fraud-transactions}", groupId = "${spring.kafka.consumer.group-id}-v2", containerFactory = "canonicalKafkaListenerContainerFactory")
    public void consumeTransactionV2(
        ...
        log.info(
                "Received v2 canonical event: txnId={}, customerId={}, amount={}, schemaVersion={}, partition={}, offset={}",
    ...
    private void validateCanonicalEvent(CanonicalFraudEvent event) {
        ...
        if (event.getCustomerId() == null) {
            throw new IllegalArgumentException(
                    "Canonical fraud event customerId is missing (customer object may be null)");
        }
        if (event.getAmount() == null) {
            throw new IllegalArgumentException(
                    "Canonical fraud event amount is missing (transaction object may be null)");
        }
        if (event.getTimestamp() == null) {
            throw new IllegalArgumentException(
                    "Canonical fraud event timestamp is missing (transaction object may be null)");
        }
    }
