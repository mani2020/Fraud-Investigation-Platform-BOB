 ...
 * Populates nested JSONB columns with rich sample data for fraud detection scenarios
 */
@Configuration
public class DataInitializer {
    ...
    CommandLineRunner initDatabase(TransactionRepository transactionRepository, TransactionService transactionService) {
        return args -> {
            ...
            String[] cities = {"New York", "London", "Mumbai", "Beijing", "São Paulo", "Moscow", "Lagos", "Berlin", "Paris", "Toronto"};
            String[] merchants = {"Amazon", "Walmart", "Target", "BestBuy", "Apple Store", "eBay", "AliExpress", "Flipkart"};
            String[] merchantCategories = {"E-COMMERCE", "RETAIL", "ELECTRONICS", "MARKETPLACE", "ONLINE_STORE"};
            ...
            for (int i = 1; i <= 20; i++) {
                try {
                    ...
                    transaction.setMerchantData(createSampleMerchantInfo(merchant, merchantCategory, i));
                    ...
                    logger.debug("Created transaction {} with nested data: txnId={}, fraudulent={}",
    ...
    private CustomerInfo createSampleCustomerInfo(String customerId, int index) {
        return CustomerInfo.builder()
            ...
            .avgTransactionAmount(BigDecimal.valueOf(500 + random.nextDouble() * 2000).setScale(2, RoundingMode.HALF_UP))
     ...
     */
    private MerchantInfo createSampleMerchantInfo(String merchantName, String category, int index) {
        return MerchantInfo.builder()
            .merchantId("MERCH-" + String.format("%05d", index))
            .merchantName(merchantName)
            .merchantCategory(category)
            .country(random.nextBoolean() ? "USA" : "UK")
            .fraudRate(BigDecimal.valueOf(random.nextDouble() * 0.1).setScale(3, RoundingMode.HALF_UP))
    ...
    private DeviceInfo createSampleDeviceInfo(int index, boolean isFraudulent) {
        return DeviceInfo.builder()
            ...
            .vpnDetected(isFraudulent && random.nextBoolean()) // VPN more likely for fraud
            .proxyDetected(isFraudulent && random.nextBoolean())
            .userAgent((random.nextBoolean() ? "Chrome" : "Safari") + "/" + (90 + random.nextInt(20)) + ".0")
            .build();
    }
     ...
     */
    private LocationInfo createSampleLocationInfo(String country, String city, int index) {
        ...
        return LocationInfo.builder()
            ...
            .city(city)
            .region(city + " Region")
            .latitude(BigDecimal.valueOf(-90 + random.nextDouble() * 180).setScale(6, RoundingMode.HALF_UP))
            .longitude(BigDecimal.valueOf(-180 + random.nextDouble() * 360).setScale(6, RoundingMode.HALF_UP))
            .ipAddress("192.168." + random.nextInt(255) + "." + random.nextInt(255))
     ...
     */
    private BehaviorMetrics createSampleBehaviorMetrics(int index, boolean isFraudulent) {
        return BehaviorMetrics.builder()
            .transactionCount24h(isFraudulent ? random.nextInt(50) + 20 : random.nextInt(15))
            .totalAmount24h(BigDecimal.valueOf(random.nextDouble() * 15000).setScale(2, RoundingMode.HALF_UP))
            .velocityScore(BigDecimal.valueOf(isFraudulent ? 70 + random.nextDouble() * 30 : random.nextDouble() * 40).setScale(2, RoundingMode.HALF_UP))
            ...
            .avgTransactionAmount(BigDecimal.valueOf(500 + random.nextDouble() * 2000).setScale(2, RoundingMode.HALF_UP))
            .maxTransactionAmount(BigDecimal.valueOf(1000 + random.nextDouble() * 9000).setScale(2, RoundingMode.HALF_UP))
            .build();
    }
     ...
     */
    private FraudSignals createSampleFraudSignals(boolean isFraudulent, int index) {
    ...
    private MetadataInfo createSampleMetadata(int index) {
        return MetadataInfo.builder()
            ...
            .userAgent((random.nextBoolean() ? "Chrome" : "Safari") + "/" + (90 + random.nextInt(20)) + ".0")
            .referrer(random.nextBoolean() ? "https://example.com" : "https://app.example.com")
            .apiVersion("v2")
