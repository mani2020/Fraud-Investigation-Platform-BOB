 ...
 * 
 * <p>This model serves as the unified internal representation for fraud analysis,
...
public class CanonicalFraudEvent {
    ...
    public String getPaymentType() {
        return transaction != null ? transaction.getPaymentType() : null;
    }
    ...
    public String getCustomerRiskLevel() {
        return customer != null ? customer.getRiskLevel() : null;
    }
    ...
    public String getMerchantRiskLevel() {
        return merchant != null ? merchant.getRiskLevel() : null;
    }

    /**
     * Check if device is trusted from nested device object.
     * 
     * @return true if device is trusted, false if not trusted or device is null
     */
    public boolean isDeviceTrusted() {
        return device != null && Boolean.TRUE.equals(device.getIsTrusted());
    }

    /**
     * Check if VPN was detected from nested fraud signals.
     * 
     * @return true if VPN detected, false otherwise
     */
    public boolean isVpnDetected() {
        return fraudSignals != null && Boolean.TRUE.equals(fraudSignals.getVpnDetected());
    }

    /**
     * Get aggregated risk score from nested fraud signals.
     * 
     * @return risk score (0-100) or null if fraud signals is null
     */
    public Integer getRiskScore() {
        return fraudSignals != null ? fraudSignals.getRiskScore() : null;
    }

    /**
     * Get velocity score from nested behavior metrics.
     * 
     * @return velocity score or null if behavior metrics is null
     */
    public BigDecimal getVelocityScore() {
        return behaviorMetrics != null ? behaviorMetrics.getVelocityScore() : null;
    }

    /**
     * Get IP address from nested device or location object.
     * Prioritizes device IP over location IP.
     * 
     * @return IP address or null if both device and location are null
     */
    public String getIpAddress() {
        if (device != null && device.getIpAddress() != null) {
            return device.getIpAddress();
        }
        return location != null ? location.getIpAddress() : null;
    }

    /**
     * Get trace ID from nested metadata object.
     * 
     * @return trace ID or null if metadata is null
     */
    ...
    public boolean hasCriticalFraudSignals() {
        ...
        return Boolean.TRUE.equals(fraudSignals.getBlacklistedMerchant())
                || Boolean.TRUE.equals(fraudSignals.getBlacklistedDevice())
                || Boolean.TRUE.equals(fraudSignals.getBlacklistedIp());
    }
    ...
    public boolean hasBehavioralAnomalies() {
        ...
        return Boolean.TRUE.equals(behaviorMetrics.getUnusualTime())
                || Boolean.TRUE.equals(behaviorMetrics.getUnusualAmount())
                || Boolean.TRUE.equals(behaviorMetrics.getUnusualLocation());
    }
}
